...
typedef struct {
	...
	int 				charm_attributes[MAX_CHARM_ATTRIBUTES]; // stores indexes to charm_attrib_t struct array that contains all info
} charm_info_T;
...
void CopyCharmInfo_FieldToPlayer(int field_index, int player_index) {
	...
	for(i = 0; i < MAX_CHARM_ATTRIBUTES; ++i)
...
bool CheckCharmAttribute(int charm_pos, int attrib_index, bool onField, int pnum) {
	int i = 0;
	if(onField) {
		for(; i < MAX_CHARM_ATTRIBUTES; ++i)
			if(Charms_On_Field[charm_pos].charm_attributes[i] == attrib_index)
				return true;
		return false;
	}
	else {
		for(; i < MAX_CHARM_ATTRIBUTES; ++i)
			if(Charms_On_Players[pnum][charm_pos].charm_attributes[i] == attrib_index)
				return true;
		return false;
	}
	return false;
...
void AddAttributeToCharm(int charm_pos, int attrib) {
	if(Charms_On_Field[charm_pos].charm_attrib_count < Charm_MaxAffixes[Charms_On_Field[charm_pos].charm_type]) {
		Charms_On_Field[charm_pos].charm_attributes[Charms_On_Field[charm_pos].charm_attrib_count++] = attrib;
	}
}
